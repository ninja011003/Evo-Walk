...
class Contraint:    
    ...
    def solve(self):
        ...
        if w1+w2!=0:
            ...
            #velocity correction
            v_rel = Vector(self.b2.velocity.x-self.b1.velocity.x,self.b2.velocity.y-self.b1.velocity.y)
            r = Vector(self.b2.position.x-self.b1.position.x,self.b2.position.y-self.b1.position.y)
            r_cap = r.normalize()
            v_rad_cap = v_rel.dot(r_cap)
            v_rad = Vector(r_cap.x*v_rad_cap,r_cap.y*v_rad_cap)
            #v_rad => velocity correction
            
            vel_cor_b1 = w1/(w1+w2) #sign convention changes...cuz we look into the change in projection and based on relative velocity defined above
            vel_cor_b2 = -w2/(w1+w2)
            ...
            if mod_dist==0:
                return self
            err = mod_dist - self.l
            _check = v_rel.dot(r_cap) * (err) # velocity corrected only if err exist after positional correction 
            if _check>0:
